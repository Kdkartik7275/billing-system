...
abstract interface class InventoryLocalDataSource {
  ...
  Future<List<StockBatchModel>> getCachedStockBatches(String productId);
...
class InventoryLocalDataSourceImpl implements InventoryLocalDataSource {
  ...
  Future<List<StockBatchModel>> getStockBatches(String productId) async {
    return stockBatchBox.values
        .where((st) => st.productId == productId && st.quantityRemaining > 0)
        .toList();
  }
  ...
  @override
  Future<List<StockBatchModel>> getCachedStockBatches(String productId) async {
    return stockBatchBox.values
        .where((st) => st.productId == productId && st.quantityRemaining > 0)
        .toList();
  }
